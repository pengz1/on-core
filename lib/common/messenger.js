...
function messengerFactory(
   ...
   /**
    ...
    Messenger.prototype.publish = function (name, routingKey, data, options) {
        ...
        .tap(function(){
            if (!options.postDataToUrl){
                return;
            }
            var userUrlList = configuration.get('userEventUrlList', []);
            postDataToUrls(
                userUrlList,
    ...
    function postDataToUrls(userUrlList, data) {
        if(_.isEmpty(userUrlList)) {
            return;
        }
        _.forEach(userUrlList, function(userUrl){
            if(_validateUrlRule(data)){
                _postDataToUrl(userUrl, data);
            }
        });
    }
    ...
    function _postDataToUrl(userUrl, data){
        var userUrlObj = url.parse(userUrl);
        var options = {
            hostname: userUrlObj.hostname,
            port: userUrlObj.port,
            path: userUrlObj.path,
            method: 'POST',
            headers: {'Content-Type': 'application/json'}
        };
        if ( userUrlObj.protocol === "https:") {
